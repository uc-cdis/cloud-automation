...
NUMBER=$( date '+%N')
gsutil mb -l us-central1 -c nearline -p $SEED_PROJECT gs://"$BUCKET-$NUMBER"
...
gcloud compute --project=$SEED_PROJECT instances create $VM --zone=$ZONE --machine-type=$TYPE --subnet=$SUBNET --network-tier=PREMIUM --maintenance-policy=MIGRATE --image=ubuntu-1604-xenial-v20190411 --image-project=ubuntu-os-cloud --scopes=https://www.googleapis.com/auth/cloud-platform
echo "All done."