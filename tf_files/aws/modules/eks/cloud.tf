...
module "jupyter_pool" {
  ...
  eks_private_subnets          = "${var.single_az_for_jupyter ? aws_subnet.eks_private[0].id : aws_subnet.eks_private.*.id}"
...
resource "aws_subnet" "eks_private" {
  ...
  cidr_block              = "${var.workers_subnet_size == 22 ? cidr_subnet(data.aws_vpc.the_vpc.cidr_block, 2 , ( 1 + count.index )) : var.workers_subnet_size == 23 ? cidrsubnet(data.aws_vpc.the_vpc.cidr_block, 3 , ( 2 + count.index )) : cidrsubnet(data.aws_vpc.the_vpc.cidr_block, 4 , ( 7 + count.index )) }"
...
resource "aws_subnet" "eks_public" {
  ...
  cidr_block              = "${var.workers_subnet_size == 22 ? cidrsubnet(data.aws_vpc.the_vpc.cidr_block, 6 , ( 4 + count.index )) : cidrsubnet(data.aws_vpc.the_vpc.cidr_block, 4 , ( 10 + count.index ))}"
