...
module "cloudwatch-events" {
  ...
  cwe_rule_name        = "${var.vpc_name}-cloudtrail-StopLogging"
...
module "alerting-lambda" {
  ...
  function_file                = ""
  lambda_function_name         = "${var.vpc_name}-security-alert-lambda"
...
module "role-for-lambda" {
  source                       = "../iam-role/"
  role_name                    = "${var.vpc_name}-security-alert-role"
...
resource "aws_iam_role_policy" "lambda_policy" {
  name                  = "${var.vpc_name}-security-alert-policy"
