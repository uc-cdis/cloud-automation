...
if [[ -f "${WORKSPACE}/${vpc_name}_output/creds.json" ]]; then # update secrets
  ...
  if ! kubectl get secrets/fence-google-app-creds-secret > /dev/null 2>&1; then
    ...
    kubectl create secret generic fence-google-app-creds-secret --from-file=fence_google_app_creds_secret.json=./apis_configs/fence_google_app_creds_secret.json
  fi

  if ! kubectl get secrets/fence-google-storage-creds-secret > /dev/null 2>&1; then
    ...
    kubectl create secret generic fence-google-storage-creds-secret --from-file=fence_google_storage_creds_secret.json=./apis_configs/fence_google_storage_creds_secret.json
  fi
