...
if [[ "$ctxNamespace" == "default" || "$ctxNamespace" == "null" ]]; then
  if (! helm status datadog -n datadog-test > /dev/null 2>&1 )  || [[ "$1" == "--force" ]]; then
    ( # subshell
      # clean up any manual deployment
      if ( g3kubectl get namespace datadog-test > /dev/null 2>&1 && ! helm status datadog -n datadog-test > /dev/null 2>&1); then
        gen3_log_info "Deleting old namespace, as it is not deployed via helm"
        g3kubectl delete namespace datadog-test
        g3kubectl create namespace datadog-test
      fi
      # create namespace if it doens't exist
      if (! g3kubectl get namespace datadog-test > /dev/null 2>&1); then
        gen3_log_info "Creating namespace datadog-test"
        g3kubectl create namespace datadog-test
      fi
      export KUBECTL_NAMESPACE=datadog-test
      if [[ -f "$(gen3_secrets_folder)/datadog/apikey" ]]; then
        if (g3kubectl get secret datadog-agent > /dev/null 2>&1); then 
          g3kubectl delete secret --namespace datadog-test datadog-agent
        fi
        g3kubectl create secret generic --namespace datadog-test datadog-agent --from-file=api-key="$(gen3_secrets_folder)/datadog/apikey"
      else
      ...
      if (! g3kubectl get secret --namespace datadog-test datadog-agent-cluster-agent > /dev/null 2>&1); then 
        # random string to secure communication between node-based agents and the cluster agent
        TOKEN=$(cat /dev/urandom | tr -dc 'a-zA-Z0-9' | fold -w 32 | head -n 1)
        g3kubectl create secret --namespace datadog-test generic datadog-agent-cluster-agent --from-literal=token="$TOKEN"
      fi
      ...
      g3kubectl apply -f "$GEN3_HOME/kube/services/datadog/argocd-application.yaml" --namespace=argocd
    )
  else
