...
# fence manifest directory config check
  ...
  if [ "$GEN3_DRY_RUN" = true ]; then
    ...
    if [[ ( "$dict_roll" = true ) || ( "$versions_roll" = true ) || ( "$portal_roll" = true )|| ( "$etl_roll" = true ) || ("fence_roll" = true)]]; then
      ...
      if [[ $slack = true ]]; then
        ...
        curl -X POST --data-urlencode "payload={\"text\": \"Gitops-sync Cron: ${resStr} - Syncing dict and images on ${tmpHostname}\", \"attachments\": [{${dictAttachment}}, {${versionsAttachment}}, {${portalAttachment}}, {${fenceAttachment}}, {${etlAttachment}}]}" "${slackWebHook}"
      fi
    else
