...
if g3k_manifest_lookup .versions.gen3workflow 2> /dev/null; then
  gen3 kube-setup-gen3workflow &
else
  gen3_log_info "not deploying gen3workflow - no manifest entry for .versions.gen3workflow"
fi
