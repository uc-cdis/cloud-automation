...
if [[ -z "$datadogPsqlPassword" || "null" == "$datadogPsqlPassword" ]]; then
  # Generate a new password for the datadog user in psql
  datadogPsqlPassword="$(gen3 random)"
...
#   postgresString+=$(cat /home/aidan/cloud-automation/kube/services/datadog/postgres.yaml | yq --arg url ${instanceArray[0]} '.instances[0].host = $url' | yq --yaml-output '.instances[0].password = "password"')
# done
