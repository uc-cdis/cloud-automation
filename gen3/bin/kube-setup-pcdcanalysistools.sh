#!/bin/bash
#
# Deploy pcdcanalysistools into existing commons - assume configs are already configured
# for pcdcanalysistools to re-use the userapi db.
...
(
  version="$(g3kubectl get secrets/pcdcanalysistools-secret -ojson 2> /dev/null | jq -r .metadata.labels.g3version)"
  if [[ -z "$version" || "$version" == null || "$version" -lt 2 ]]; then
    g3kubectl delete secret pcdcanalysistools-secret > /dev/null 2>&1 || true
    g3kubectl create secret generic pcdcanalysistools-secret "--from-file=wsgi.py=${GEN3_HOME}/apis_configs/pcdcanalysistools_settings.py" "--from-file=${GEN3_HOME}/apis_configs/config_helper.py"
    g3kubectl label secret pcdcanalysistools-secret g3version=2
  fi
)
...
if [[ -z "$JENKINS_HOME" && -f "$(gen3_secrets_folder)/creds.json" ]]; then
  ...
  #
  # Create the 'pcdcanalysistools' and 'peregrine' postgres user if necessary
  #
...
# deploy pcdcanalysistools 
gen3 roll pcdcanalysistools
g3kubectl apply -f "${GEN3_HOME}/kube/services/pcdcanalysistools/pcdcanalysistools-service.yaml"
# gen3 roll pcdcanalysistools-canary || true
# g3kubectl apply -f "${GEN3_HOME}/kube/services/pcdcanalysistools/pcdcanalysistools-canary-service.yaml"
