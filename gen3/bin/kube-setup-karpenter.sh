...
gen3_deploy_karpenter() {
  ...
  if [[ -z $(g3kubectl get namespaces | grep karpenter) ]]; then
  ...
  ssh_keys=$(cat ${GEN3_HOME}/files/authorized_keys/ops_team)
  g3k_manifest_filter ${GEN3_HOME}/kube/services/karpenter/nodeTemplateDefault.yaml VPC_NAME ${vpc_name} | g3kubectl apply -f -
