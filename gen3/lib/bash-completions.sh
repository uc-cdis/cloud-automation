...
gen3_completions() {
  if [[ $COMP_CWORD -lt 2 ]]; then
  ...
  elif [[ ${COMP_CWORD} -eq 2 && "${COMP_WORDS[1]}" == "dashboard" ]]; then
    COMPREPLY=($(compgen -W "prefix publish gitops-sync" "${COMP_WORDS[2]}"))
  elif [[ ${COMP_CWORD} -eq 2 && "${COMP_WORDS[1]}" == "gitops" ]]; then
    COMPREPLY=($(compgen -W "filter configmaps rsync sshlist sync repolist taglist dotag tfplan tfapply" "${COMP_WORDS[2]}"))
    ...
    # see https://unix.stackexchange.com/questions/151118/understand-compgen-builtin-command
