...
(
  while true; do curl -s http://127.0.0.1:9117/metrics > /var/www/metrics/metrics.txt && curl -s http://127.0.0.1:9113/metrics >> /var/www/metrics/metrics.txt && curl -s http://127.0.0.1:4040/metrics >> /var/www/metrics/metrics.txt; sleep 10; done
) &
