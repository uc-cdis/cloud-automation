...
resource "aws_launch_configuration" "eks_launch_configuration" {
  ...
  user_data_base64            = base64encode(templatefile(file("${path.module}/../../../../flavors/eks/${var.bootstrap_script}"),{eks_ca = var.eks_cluster_ca, eks_endpoint = var.eks_cluster_endpoint, eks_region = data.aws_region.current.name, vpc_name = var.vpc_name, ssh_keys = templatefile(file("${path.module}/../../../../files/authorized_keys/ops_team"),{}), nodepool = var.nodepool, kernel = var.kernel, activation_id = var.activation_id, customer_id = var.customer_id}))
